...
5. The committed translations_data.h matches translations.json.
"""
import json
import re
...
INCLUDE_DIR = Path(__file__).resolve().parent.parent / "src" / "include"
TRANSLATIONS_HEADER = INCLUDE_DIR / "translations_data.h"
TRANSLATIONS_TEMPLATE = INCLUDE_DIR / "translations_data.h.in"
...
def _embedded_translations():
    """Extract the JSON embedded in the committed translations_data.h."""
    header = TRANSLATIONS_HEADER.read_text(encoding="utf-8")
    match = re.search(r'R"TJSON\((.*?)\)TJSON"', header, re.DOTALL)
    assert match, (
        f"{TRANSLATIONS_HEADER.name} does not contain the expected "
        'R"TJSON(...)TJSON" raw string literal'
    )
    return json.loads(match.group(1))


def test_header_is_in_sync_with_json(translations):
    """The committed translations_data.h matches translations.json.

    The header is generated by CMake from translations.json, but it is also
    committed to the repository. A commit that edits translations.json without
    re-running the CMake configure step leaves the two out of sync -- which is
    exactly what happened between 2026-07-22 and 2026-08-04.

    Builds recover on their own because translations.json is listed in
    CMAKE_CONFIGURE_DEPENDS, so this guards the repository contents rather than
    the build: it keeps anyone reading src/include/ from seeing a stale map.
    """
    embedded = _embedded_translations()
    assert embedded == translations, (
        "translations_data.h is out of sync with translations.json. "
        "Re-run the CMake configure step (or regenerate the header from "
        "translations_data.h.in) and commit the result."
    )


def test_header_matches_template():
    """translations_data.h is the template with the JSON substituted verbatim.

    Catches hand-edits to the generated header, which the file's own banner
    forbids but nothing enforced until now.
    """
    template = TRANSLATIONS_TEMPLATE.read_text(encoding="utf-8")
    payload = TRANSLATIONS_FILE.read_text(encoding="utf-8").rstrip("\n")
    expected = template.replace("@TRANSLATIONS_JSON_CONTENT@", payload)

    actual = TRANSLATIONS_HEADER.read_text(encoding="utf-8")
    assert actual.splitlines() == expected.splitlines(), (
        "translations_data.h differs from translations_data.h.in with "
        "translations.json substituted. Do not edit the header by hand."
    )
