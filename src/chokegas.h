...
#include "PropFlu.h"
         ...
         static double ventCR[5][9][10] =//alteracao3
           ...
           {{0.6511, 0.6491, 0.6471, 0.6453, 0.6436, 0.6420, 0.6405, 0.6391, 0.6379, 0.6368},  // sgg=0.95,   P=  0 bar
            ...
            {1.1069, 1.0285, 0.9614, 0.9048, 0.8575, 0.8183, 0.7860, 0.7595, 0.7378, 0.7200}}};// sgg=0.95,   P=400 bar//alteracao3
...
class ChokeGas{
        ...
        ChokeGas(const ProFlu vflui=ProFlu(),
            ...
            double vpcalib=1.,double vtcalib=1,const double vcdliq=1.,const double vfrecliq= 0);//construtor default//alteracao2
        ChokeGas(const ChokeGas&); //construtor por c�pia
  	    ChokeGas& operator=(const ChokeGas&);

  	    double massica(int fluido=0, double salin=0.);//alteracao2
  	    double tgarg();
  	    double razcrit();
  	    double fraiz(double kad, double rt, double mass, double rp);
  	    double derraiz(double kad, double rp);
  	    double newton(double kad, double rt, double mass, double rp);
  	    double razpres(double mass, double rp);
  	    double razpresSimples(double mass, double rp);
  	    double FMachVenturi(double Mach, double kad);
  	    double DeriFMachVenturi(double Mach, double kad);
  	    double newtonMachVenturi(double kad);
  	    double razpresVenturi();
  	    double massicaVenturi();
  	    double MasEspFlu(double pres, double temp, double salin) const; // alteracao2
