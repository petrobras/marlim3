...
template<class T> Vcr<T>::Vcr(const Vcr& v){//construtor de c�pia
...
template<class T> void Vcr<T>::resize(int n){//redimensionamento de vetor
    ...
    for (int i=v.lenth; i<lenth; i++) vr[i]= 0; //espa�o redimensionado todo inicializado com zeros
}
...
template<class T> Vcr<T>& Vcr<T>::operator =(const Vcr& v){//iguala dois vetores
      if(lenth>0){
       if(this!=&v){//verifica se se est� igualando o mesmo vetor
        if(lenth!=v.lenth){
        	//cout<<"dimens�es de vetores diferem, imposs�vel igualar";//mesmo tamanho de vetores?
        	if(vr)delete[] vr;
        	vr = new T [lenth=v.lenth];
        }
        #pragma omp parallel for num_threads(nthrdMatriz)
...
template<class T> ostream& Vcr<T>::saida(ostream& s, const Vcr& v) const{//sa�da
...
template<class T> T Vcr<T>::maxnorm() const{ 
	T nm = fabs(vr[0]);
	for (int i =1;i<lenth;i++) nm=max<T>(nm,fabs(vr[i]));//busca o maior elemento de vr em m�dulo
	return nm;
}

template<class T> T Vcr<T>::onenorm() const{ 
	T nm = fabs(vr[0]);
	for (int i =1;i<lenth;i++) nm += fabs(vr[i]);//soma os m�dulos dos elementos de vr
	return nm;
}
...
template<class T> Vcr<T>& Vcr<T>::operator +=(const Vcr& v){//soma o pr�prio vetor com outro vetor e iguala com o primeiro
	  if(lenth!=v.lenth) cout<<"dimens�es de vetores diferem, imposs�vel igualar e somar";//mesmo tamanho de vetores?
      #pragma omp parallel for num_threads(nthrdMatriz)
      for (int i=0;i<lenth;i++) vr[i]+=v[i];//igualando vetores, lembre-se da sobrecarga do operador [] em v[i]
	return *this;
}

template<class T> Vcr<T>& Vcr<T>::operator +=(T a){//soma o pr�prio vetor com um escalar
#pragma omp parallel for num_threads(nthrdMatriz)
        for (int i=0;i<lenth;i++) vr[i]+=a;//igualando vetores, lembre-se da sobrecarga do operador [] em v[i]
	return *this;
}

template<class T> Vcr<T>& Vcr<T>::operator -=(const Vcr& v){//subtrai o pr�prio vetor com outro vetor e iguala com o primeiro
	  if(lenth!=v.lenth) cout<<"dimens�es de vetores diferem, imposs�vel igualar e somar";//mesmo tamanho de vetores?
#pragma omp parallel for num_threads(nthrdMatriz)
      for (int i=0;i<lenth;i++) vr[i]-=v[i];//igualando vetores, lembre-se da sobrecarga do operador [] em v[i]
	return *this;
...
template<class T> Vcr<T>& Vcr<T>::operator -=(T a){//subtrai o pr�prio vetor com um escalar
#pragma omp parallel for num_threads(nthrdMatriz)
      for (int i=0;i<lenth;i++) vr[i]-=a;//igualando vetores, lembre-se da sobrecarga do operador [] em v[i]
	return *this;
}

template<class T> Vcr<T> Vcr<T>::operator -(const Vcr<T>& v) const{ //subtra��o de vetores
       if (lenth != v.lenth) cout<<"tamanho de vetores n�o confere\n";
...
template<class T> Vcr<T> Vcr<T>::operator-(T a) const{ //subtra��o de vetor com escalar
       Vcr<T> sum= *this;//c�pia de vetores
	   sum -=a;
	   return sum;
}
...
template<class T> Vcr<T> Vcr<T>::operator*(const Vcr<T>& v) const{ //multiplicacao de um vetor por outro
  ...
  Vcr<T> tm(n);
#pragma omp parallel for num_threads(nthrdMatriz)
  for (int i = 0; i < n; i++) tm[i]=vr[i]*v[i];//observe que na multiplicacao, ao contr�rio do "dot", o resultado ainda � um vetor e n�o um escalar
  return tm;
}
...
/*template<class T> T dot(const Vcr<T>& v1, const Vcr<T>& v2){//calcula produto interno
	if (v1.lenth!=v2.lenth) cout<<"tamanho de vetores n�o confere\n";
	T tm = v1[0]*v2[0];
...
double dot(const Vcr<double>& v1, const Vcr<double>& v2){//calcula produto interno
       	if (v1.size()!=v2.size()) cout<<"tamanho de vetores n�o confere\n";
        int tam=v1.size();
        //double tm = v1[0]*v2[0];
        double tm = 0;
