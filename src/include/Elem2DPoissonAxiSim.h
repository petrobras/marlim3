...
#ifndef elem2dPoissonPOISSONAxiSim_H_
#define elem2dPoissonPOISSONAxiSim_H_
 ...
 */
class elem2dPoissonAxiSim {
  public:
    detCCPoisson CC;           // Boundary-condition data.
    elementoPoissonAxiSim cel2D;     // Data associated with the current element.
    int nvizinho;              // Number of neighboring elements.
    elementoPoissonAxiSim **vizinho; // Pointers to neighboring elements.
     ...
     */
    elem2dPoissonAxiSim(varGlob1D *Vvg1dSP = 0, double **xcoor = 0, int **noEle = 0,
    		int *tipo = 0, double *atributo = 0, int nVert = 0, int nele = 0, int nno = 0,
            int vperm = 1, int vtrans = 0, int i = 0,int vacopD=0, double vtemp = 0.,
			double vfluxCal = 0., double vcond = 0., double vcp = 0.,
            double vrho = 0., double vdt = 0.);

    //! Copy constructor.
    elem2dPoissonAxiSim(const elem2dPoissonAxiSim &);

    //! Copy-assignment operator.
    elem2dPoissonAxiSim &operator=(const elem2dPoissonAxiSim &);
    ...
    double escalarAreaAxiSim(double *v1,int dimV, int indFace);
